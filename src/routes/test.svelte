<script>
  ...
  function handleVideoInputBlur() {
    // If no video URL is entered, just close the input and keep the previous content
    if (!videoUrl) {
      setTimeout(() => {
        showVideoInput = false;
        videoUrl = "";
        activeFeatureButton = null;
        updateFeatureButtonColors();
      }, 100);
    }
  }
  ...
  function updateFeatureButtonColors() {
    const videoButton = document.querySelector('.ql-video');
    const imageButton = document.querySelector('.ql-image');
    
    if (videoButton && imageButton) {
      // Önce her iki butonu da gri yap
      videoButton.style.color = '#5F5F5F';
      imageButton.style.color = '#5F5F5F';
      
      // Eğer currentFeatureContent varsa, ilgili butonu mavi yap
      if (currentFeatureContent) {
        if (currentFeatureContent.type === 'video') {
          videoButton.style.color = '#6792FF';
        } else if (currentFeatureContent.type === 'image') {
          imageButton.style.color = '#6792FF';
        }
      }
    }
  }

  // currentFeatureContent değiştiğinde buton renklerini güncelle
  $: {
      ...
      featureEditor = new Quill('#feature-editor', {
        modules: {
          toolbar: {
            ...
            handlers: {
              image: function() {
                // Eğer video içeriği varsa önce onu temizle
                if (currentFeatureContent?.type === 'video') {
                  currentFeatureContent = null;
                  showVideoInput = false;
                  videoUrl = "";
                  // Feature element'i default haline getir
                  const featureElement = document.querySelector(".feature-element");
                  if (featureElement) {
                    featureElement.innerHTML = `<img src="${featureSrc}" alt="Feature content" class="feature-image" style="object-fit: cover; width: 100%; height: 100%; display: block;"/>`;
                  }
              ...
              video: function() {
                // Eğer resim içeriği varsa önce onu temizle
                if (currentFeatureContent?.type === 'image') {
                  currentFeatureContent = null;
                  if (featureEditor) {
                    featureEditor.setContents([]);
                  }
      ...
      featureEditor.on('text-change', function() {
        ...
        if (featureElement) {
          if (img && currentFeatureContent?.type === 'image') {
            featureElement.innerHTML = `<img src="${currentFeatureContent.data}" alt="Feature content" class="feature-image" style="object-fit: cover; width: 100%; height: 100%; display: block;"/>`;
...
<style>
  * {
  ...
  /* Seçili durum için özel stiller */
