<script>
  ...
  function handleVideoEmbed() {
    console.log('handleVideoEmbed çağrıldı', featureState.url);
    if (featureState.url) {
      try {
        // Önce state'i temizle ki updateFeatureContent çalışsın
        updateFeatureState({
          content: null
        });

        // Feature editöre video ekle
        featureEditor.focus();
  ...
  function handleSaveReference(event) {
    ...
    savedReferences = [...savedReferences, reference]; // Yeni referansı ekle
  ...
  onMount(() => {
    if (typeof window !== "undefined") {
                ...
                input.onchange = () => {
                  ...
                  if (file) {
                    ...
                    reader.onload = (e) => {
                      ...
                      this.quill.insertEmbed(0, 'image', e.target.result);
                    };
      ...
      featureEditor.on('text-change', () => {
        ...
        isProcessing = true;
        console.log('text-change event tetiklendi');
        updateFeatureContent(featureEditor);
        
        // Bir sonraki event loop'ta reset et
        setTimeout(() => {
          isProcessing = false;
        }, 0);
      });
  ...
  const featureSrc =
    "https://placehold.co/500x300?text=No+Image+Selected";
</script>
  ...
  <div class="resource-section">
    ...
    <div class="resource-form">
      <div class="feature-element">
        ...
        {:else}
          <img
            src={currentFeatureContent?.type === 'image' ? currentFeatureContent.data : featureSrc}
            alt="Feature content"
            class="feature-image"
          />
        {/if}
